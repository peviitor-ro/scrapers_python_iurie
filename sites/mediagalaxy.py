...
def scraper():
    ...
    response = GetCustumRequest(url, headers, payload)
   
    job_list = []
    for job in response.find_all("div", class_="border rounded px-8"):

        title = job.find(
            'h2', class_="flex-shrink-0 mb-3 md:mb-0 md:w-64 md:pr-6 font-medium capitalize").text
        link = title.replace(" ", "-")
        location = job.find(
            'div', class_="w-full mb-2 md:mb-0").text.split(':')[1].strip().split(', ')
        county_finish = ["Cluj"if city == "Cluj" else get_county_json(city)[
