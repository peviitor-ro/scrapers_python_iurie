...
def scraper():
    ...
    for job in post_data["results"][0]["hits"]:
        locations = job["document"]["data"]["locations"]

        if len(locations) > 1:
            cities = [loc["city"] for loc in locations]
            counties_raw = [loc["state"] for loc in locations]
        else:
            cities = locations[0]["city"]
            counties_raw = locations[0]["state"]

        city = "Bucuresti" if "Bucharest" in cities else cities
        county = _first_county(counties_raw) if isinstance(cities, list) else counties_raw
